...
const LabourWelfareFundDetails = () => {
  ...
  // Hardcoded key LWF state acts for generating standard dynamic routing links below
  const targetLwfStates = [
    { name: "Maharashtra Labour Welfare Fund Act, 1953", routeSlug: "maharashtra-labour-welfare-fund" },
    { name: "Karnataka Labour Welfare Fund Act, 1965", routeSlug: "karnataka-labour-welfare-fund" },
    { name: "Gujarat Labour Welfare Fund Act, 1953", routeSlug: "gujarat-labour-welfare-fund" },
    { name: "Haryana Labour Welfare Fund Act, 1965", routeSlug: "haryana-labour-welfare-fund" },
    { name: "Tamil Nadu Labour Welfare Fund Act, 1972", routeSlug: "tamil-nadu-labour-welfare-fund" },
    { name: "Delhi Labour Welfare Fund Rules, 1997", routeSlug: "delhi-labour-welfare-fund" }
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 pb-20 overflow-x-hidden">
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-4">
              <div className="p-2.5 bg-rose-500 rounded-xl shadow-lg shadow-rose-500/20 flex-shrink-0">
                <HeartPulse size={24} className="text-white" />
              </div>
              <h1 className="text-2xl lg:text-4xl font-black tracking-tight uppercase break-words leading-none">
                {data.title || "Labour Welfare Fund Laws (State LWF Applicability)"}
              </h1>
            </div>
      ...
      <main className="w-full px-4 lg:px-8 -mt-4 relative z-20">
        <div className="grid grid-cols-12 gap-6 lg:gap-10">
          ...
          <div className="col-span-12 xl:col-span-9 bg-white rounded-[2rem] lg:rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 p-6 lg:p-10 space-y-8 min-w-0">
            ...
            <section id="state-lwf-portal" className="w-full min-w-0 bg-slate-50/70 border border-slate-200/60 p-6 lg:p-8 rounded-[2rem]">
              <SectionHeader icon={<MapPin className="text-rose-600" />} title="State-Wise Welfare Boards & Contribution Slabs" />
              <p className="text-sm text-slate-500 pl-6 lg:pl-10 mb-6 font-medium">
                Labour Welfare Fund contribution frequencies (Monthly, Half-Yearly, or Annual) and deduction components vary widely across state boards. Select a region below to review custom employer-employee sharing ratios, structural exemptions, and due dates:
              </p>
              
              <div className="pl-6 lg:pl-10 grid gap-3 sm:grid-cols-1 md:grid-cols-2">
                {targetLwfStates.map((state, idx) => (
                  <Link
                    key={idx}
                    to={`/elibrary/${state.routeSlug}`}
                    className="flex items-center justify-between p-4 bg-white hover:bg-rose-50/30 border border-slate-200/80 hover:border-rose-500/30 rounded-xl transition-all shadow-sm group"
                  >
                    <span className="text-[13px] font-bold text-slate-700 group-hover:text-rose-600 transition-colors tracking-tight">
                      {state.name}
                    </span>
                    <ArrowRight size={16} className="text-slate-400 group-hover:text-rose-500 group-hover:translate-x-1 transition-all flex-shrink-0 ml-2" />
                  </Link>
                ))}
              </div>
            </section>
            ...
            {data.includedActs && data.includedActs.length > 0 && (
              <section id="detailed-breakdown" className="space-y-4 w-full min-w-0">
                <SectionHeader icon={<Gavel className="text-orange-600" />} title="Act Specific Enactments" />
            ...
            <section className="bg-blue-50/50 p-6 lg:p-8 rounded-[2rem] border border-blue-100 w-full min-w-0">
              <SectionHeader icon={<FileText className="text-orange-600" />} title="02. Official Bare Act Provisions" />
            ...
            <div className="grid lg:grid-cols-2 gap-8 pt-2 w-full min-w-0">
              <section className="border-l-4 border-purple-100 pl-6 lg:pl-10 min-w-0">
                <SectionHeader icon={<Gavel className="text-purple-600" />} title="03. Recent Rate Amendments" />
              ...
              <section className="border-l-4 border-emerald-100 pl-6 lg:pl-10 min-w-0">
                <SectionHeader icon={<ClipboardCheck className="text-emerald-600" />} title="04. Statutory Board Rules" />
            ...
            <section id="practical-implementation-section" className="w-full min-w-0 border-l-4 border-amber-100 pl-6 lg:pl-10">
              <SectionHeader icon={<AlertCircle className="text-amber-600" />} title="05. Payroll Integration & Compliance" />
          ...
          <aside className="col-span-12 xl:col-span-3 space-y-6">
                ...
                {targetLwfStates.map((state, i) => (
                  <li key={i}>
                    ...
                    >
                      ...
                      <span className="text-[12px] font-bold text-slate-700 group-hover:text-rose-600 underline decoration-transparent group-hover:decoration-rose-500 transition-all">
            ...
            <div className="bg-[#0B1538] p-6 rounded-[2.5rem] text-white shadow-xl">
              ...
              <p className="text-[11px] text-slate-300 leading-relaxed font-medium mb-4">
                Scroll down to review the isolated FAQ module for answers on state board filing processes regarding LWF.
              </p>
              <button 
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .rich-text-area a { color: #f43f5e; text-decoration: underline; font-weight: 800; }
  ...
  return (
    <div className="border-b border-slate-100 pb-2.5 last:border-none last:pb-0">
      <button
        ...
        className="w-full flex justify-between items-start text-left gap-2 py-1 text-slate-800 font-bold text-[12px] tracking-tight hover:text-rose-500 transition-colors group"
      >
        <span className="leading-tight">Q: {faq.question}</span>
        <ChevronDown 
          size={14} 
          className={`mt-0.5 text-slate-400 group-hover:text-rose-500 transition-transform duration-200 flex-shrink-0 ${isOpen ? "rotate-180" : ""}`} 
        />
      </button>
      ...
      >
        <div className="overflow-hidden">
          <div 
            className="text-slate-500 text-[11px] leading-relaxed pl-3 border-l-2 border-rose-500/30 py-0.5"
...
export default LabourWelfareFundDetails;